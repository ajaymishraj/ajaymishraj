<div align="center">

## Ajay Mishra

1st-year B.Tech CSE (AI) @ NIET, Greater Noida · Building tools real users depend on

[![Portfolio](https://img.shields.io/badge/Portfolio-ajxmishra1.web.app-185FA5)](https://ajxmishra1.web.app)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2)](https://linkedin.com/in/ajay-mishra-558a62389/)
[![Email](https://img.shields.io/badge/Email-ajaymishraj25%40gmail.com-D85A30)](mailto:ajaymishraj25@gmail.com)

</div>

---

### About me

I build Firebase web tools that real users actually depend on — not toy projects.
Three production apps deployed before completing my first year of college,
serving 500+ users across colleges.

---

### Live projects

| Project | What it does | Stats |
|---------|-------------|-------|
| [**Bunkoo Meter**](https://bunkoo-meter.web.app) | PWA attendance tracker — AI Insights, friends leaderboard, holiday management | 500+ users · 6,300+ events · 3,100+ views |
| [**Finacy**](https://finacy-91d2a.web.app) | Offline-first business mgmt — inventory, sales, rentals · 100% on-device privacy | Used by local enterprises |
| [**NIET Lost & Found**](https://niet-lostfound.web.app) | Campus lost item portal — photo uploads, admin dashboard · no login for students | NIET campus |

---

### Tech stack

```
Frontend    HTML · CSS · JavaScript · PWA
Cloud       Firebase (Auth · Hosting · Firestore · Storage) · IndexedDB
Languages   C · C++ · Python
AI          Generative AI · Prompt Engineering
```

---

### Certifications

- Master Generative AI & Tools — Udemy Business · Jan 2026
- Next Gen Technologies — Infosys Springboard · Feb 2026
- Claude 101 Workshop — Outskill · May 2026

---

![GitHub stats](https://github-readme-stats.vercel.app/api?username=ajaymishraj&show_icons=true&hide_border=true&count_private=true)

---

*🎯 Open to Summer 2026 web development internships → ajaymishraj25@gmail.com*
